...
export class AwsIamAdapter implements CloudServiceAdapter {
    ...
    async create(input: CreateResourceInput): Promise<CloudResource> {
        ...
        if (kind === 'roles') {
            const document = requiredJson(input.values.assumeRolePolicyDocument, 'assumeRolePolicyDocument', 'roles')
    ...
    async delete(id: string): Promise<void> {
        const {kind, identifier} = parseId(id)
        const singular = IAM_KIND_SINGULAR[kind]
        ...
        try {
            ...
            if (isNoSuchEntity(error)) throw new NotFoundError(`IAM ${singular} ${identifier} not found`, {cause: error})
            if (isDeleteConflict(error)) {
                throw new ConflictError(
                    `IAM ${singular} ${identifier} still has attached resources. Detach or remove them before deleting.`,
                    {cause: error},
                )
            }
            throw error
    ...
    private async sendList(kind: IamKind, marker: string | undefined): Promise<{identities: Identity[]; marker?: string}> {
...
function parseId(id: string): {kind: IamKind; identifier: string} {
    ...
    if (!kind || !identifier) {
        throw new ValidationError(`IAM id must start with ${Object.keys(SINGULAR_TO_KIND).join(', ')}, got "${prefix}"`)
    }
...
function optionalPath(value: unknown): string | undefined {
    ...
    if (path.length > IAM_PATH_MAX_LENGTH || !new RegExp(IAM_PATH_PATTERN).test(path)) {
