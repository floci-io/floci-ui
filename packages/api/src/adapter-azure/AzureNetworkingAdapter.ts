 ...
 *                 (the runtime treats PUT as an upsert; the adapter GETs first
 *                  and rejects with ConflictError rather than modifying an
 *                  existing VNet, since the UI only exposes "Create")
...
function optionalSubnet(
    ...
    if (!cidrContains(addressPrefix, prefix)) {
        throw new ValidationError(`subnetPrefix must be contained within addressPrefix ${addressPrefix}`)
    }
    return {name, prefix}
}

/** Whether every address in `inner` falls within `outer`. Both must already be valid CIDRs. */
function cidrContains(outer: string, inner: string): boolean {
    const o = cidrRange(outer)
    const i = cidrRange(inner)
    return i.prefix >= o.prefix && i.start >= o.start && i.end <= o.end
}
...
function cidrRange(cidr: string): {start: number; end: number; prefix: number} {
    ...
    const size = hostBits === 0 ? 0 : 2 ** hostBits - 1
    return {start, end: start + size, prefix}
}
